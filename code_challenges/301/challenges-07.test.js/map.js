...
// REFERENCE: ** is the exponentiation operator and is the equivalent of Math.pow.
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const forEachTwoToThe = (arr) => {
  // Solution code here...
    const newArrTwo = [];
    arr.forEach((num) => { newArrTwo.push(Math.pow(2, num))})
    return newArrTwo;
};
...
const extractStats = (arr) => {
  // Solution code here...
  let snorlaxStatsMap = arr.map((val) => {
    //   let total = (val.effort + val.baseStats);
    //   return (val.stat.name , total);
    // return val.stat.name, (val.effort + val.baseStats);
    return {
        name: val.stat.name,
